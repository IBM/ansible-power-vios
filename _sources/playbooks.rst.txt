...
The sample playbooks that are **included** in the **IBM Power Systems VIOS collection**
...
**IBM Power Systems VIOS collection**. The sample playbook can be run with the
...
refer to the installation path as ``~/.ansible/collections/ansible_collections/ibm/power_vios``.
...
.. code-block:: yaml
   ...
   vioserver:
     hosts:
       vio:
         ansible_host: vios_target_address
         ansible_user: vios_target_username
...
The value for the property **vios_target_username** is the user name to use when
...
.. _sample inventory file:
   https://github.com/IBM/ansible-power-vios/blob/dev-collection/playbooks/inventory.yml
...
.. _all.yml:
   https://github.com/IBM/ansible-power-vios/blob/dev-collection/playbooks/group_vars/all.yml

.. _vioserver.yml:
   https://github.com/IBM/ansible-power-vios/blob/dev-collection/playbooks/group_vars/vioserver.yml
...
collection: ``~/.ansible/collections/ansible_collections/ibm/power_vios/playbooks/``.
...
``~/.ansible/collections/ansible_collections/ibm/power_vios/playbooks/``.
...
The **IBM Power Systems VIOS collection** uses the standard Ansible log system
that is using the syslog subsystem on the managed nodes.

To activate VIOS syslog you can update the configuration file `/etc/syslog.conf`
...
.. code-block:: sh
   ...
   May  6 03:28:27 nimmaster user:info ansible-nim: Invoked with resource=None force=False description=None script=damien_script lpp_source=None action=script asynchronous=True location=None device=None group=None operation=None targets=['vioclient01']
...
.. code-block:: sh
   ...
   $ refresh -s syslogd
   $ ANSIBLE_DEBUG=1 ansible-playbook -M plugins/modules ./demo_vio.yml -vvv
