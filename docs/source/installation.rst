...
You can install the **IBM Power Systems VIOS collection** using one of these options:
...
Here are a few examples of installing the **IBM Power Systems VIOS collection**:

.. code-block:: sh

   $ ansible-galaxy collection install ibm.power_vios
   $ ansible-galaxy collection install -f ibm.power_vios
   $ ansible-galaxy collection install --force ibm.power_vios
...
.. code-block:: sh
   ...
   Installing 'ibm.power_vios:1.0.0' to '/Users/user/.ansible/collections/ansible_collections/ibm/power_vios'
...
.. code-block:: sh
   ...
   $ ansible-galaxy collection install ibm.power_vios -p /home/ansible/collections
...
directory of the IBM ansible-power-vios Git repository. The archives can be
installed locally without having to use Hub or Galaxy.

To install a build from the ansible-power-vios Git repository:

   1. Obtain a local copy from the Git repository:

      .. note::
         * Collection archive names will change depending on the release version.
         * They adhere to this convention **<namespace>-<collection>-<version>.tar.gz**, for example, **ibm-power_vios-1.0.0.tar.gz**
   ...
   2. Install the local collection archive:
      ...
      .. code-block:: sh
         ...
         $ ansible-galaxy collection install ibm-power_vios-1.0.0.tar.gz
      ...
      installation path, for example, ``ansible-galaxy collection install ibm-power_vios-1.0.0.tar.gz -p /home/ansible/collections``.
